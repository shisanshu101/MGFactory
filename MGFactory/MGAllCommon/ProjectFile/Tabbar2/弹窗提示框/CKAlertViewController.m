...
- (void)creatAllButtons {
    ...
    for (int i=0; i<self.actions.count; i++) {
       ...
       // if(self.actions.count==2){
            [btn setTitleColor:RGBA(0, 150, 230) forState:UIControlStateNormal];
...
- (void)updateAllButtonsFrame {
    ...
    for (int i=0; i<self.actions.count; i++) {
        ...
        if(self.actions.count==2){
            UILabel * lab = [_contentView viewWithTag:1200];
            lab.frame = CGRectMake(buttonWidth, buttonY, 1, buttonY);
